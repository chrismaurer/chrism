...
order_tags = ["account", "order_capacity", "sText2", "sText1", "sMemo", "PARTY_ROLE_CLIENT_ID",
              "PARTY_ROLE_INVESTMENT_DECISION_MAKER", "PARTY_ROLE_EXECUTING_TRADER", "text_tt", "order_origination",
              "order_attribute_type", "PARTY_ROLE_QUALIFIER_NATURAL_PERSON", "PARTY_ROLE_CLEARING_ACCOUNT",
              "ORDER_ATTRIBUTE_TYPE_LIQUIDITY_PROVISION_ACTIVITY_ORDER"]
...
def verify_otd_data(log_message):
    # print log_message.keys()
    for element in log_message.iteritems():
        # print "element:", element
        for order_tag in order_tags:
            if order_tag in str(element):
