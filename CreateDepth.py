...
products = priceSession.getProducts(prodName='CN', prodType=aenums.TT_PROD_FUTURE)
    ...
    for product in products:
        ...
        if "STOCK" not in str(product.prod_type):
            if "FUTURE" not in str(product.prod_type):
                pricey = 3
            contracts = priceSession.getContracts(product, contractKeys=["CNM18", "CNN18"])
            for contract in contracts:
                ...
                if pricey is not None and trading_status != 6:
                    for side in [aenums.TT_SELL, aenums.TT_BUY]:
                        ...
                        while depth_level <= 4:
                            if "SELL" in str(side):
                                pricey = (cppclient.TTTick.PriceIntToInt(pricey, contract, +random.randint(1, 6)))
                            else:
                                pricey = (cppclient.TTTick.PriceIntToInt(pricey, contract, -random.randint(1, 6)))
