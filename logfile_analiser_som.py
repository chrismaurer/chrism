...
class logfile_analiser():
        ...
        for OrderBookID in OrderBookIDs:
            ...
            for line in inputFile.readlines():
                if 'Contract Created' in line or '_DEFINITIONS' in line:
                    if '%s]' % OrderBookID in line and 'CPA9.25C9' in line:
                        count = 0
                        found = True
                if found:
                    if 'msgType' in line and '[id:%s]' % OrderBookID in line and 'CPA9.25C9' in line:
                        print line
                        in_file = True
                        count += 1
    ...
    def parseLogfile02(self):
        ...
        obID_seqs = [['0', '39091'], ['0', '38875'], ['0', '42965'], ['0', '6841']]

        logfile = self.optmenu()

        for obID_seq in obID_seqs:
            print '-'*40, 'Session 1 - Test case 2: Sec. C Interpretation of Status Data: Market Status', '-'*40
            inputFile = open(logfile, 'r')
            in_file = False
            for line in inputFile.readlines():
